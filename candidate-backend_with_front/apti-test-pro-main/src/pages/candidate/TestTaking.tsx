...
export default function TestTaking() {
  ...
  const runCode = async () => {
    if (!currentCodingAnswer) return;
    
    setCodeOutput("Executing...\n");
    try {
      const res = await executeCode(
        (currentQuestion as typeof test.codingQuestions[0]).language,
        currentCodingAnswer.code,
        codeInput
      );

      if (res.ok && res.data?.run) {
        const output = res.data.run.stdout || res.data.run.output || res.data.run.stderr;
        const status = res.data.status;
        
        // Display output with status indicator
  ...
  const handleSubmitCode = async () => {
    if (!currentCodingAnswer || !test) return;
  ...
  const confirmSubmitCode = async () => {
    ...
    if (!currentCodingAnswer || !test || !session) return;
    
    const { useCandidateTestStore } = await import("@/store/candidateTestStore");
    ...
    try {
      ...
      if (candidateId) {
        await apiPost('/test-answer', {
          ...
          code_submission: currentCodingAnswer.code,
          code_output: codeOutput,
          stdin: codeInput,
          stdout: codeOutput,
          code_status: codeOutput.includes('✗') ? 'error' : codeOutput.includes('✓') ? 'success' : 'pending',
          code_passed: codeOutput.includes('✓'),
          time_spent_seconds: Math.floor((Date.now() - (new Date(session.startedAt || new Date()).getTime())) / 1000),
        });
      }
      ...
      toast({
        ...
        description: "Failed to submit code. It may have been saved locally.",
        variant: "destructive",
      });
    }
  };
